...
function main()
{
	...
	if(bDebug)
		WScript.Echo("원본: " + strSourceFolder);

	if(!FileSys.FolderExists(strSourceFolder))
	{
		WScript.Echo("오류: 마법사 폴더를 찾을 수 없습니다. (원본: " + strSourceFolder + ")");
		return;
	}

	MessageBox(WSShell, "Setup은 설치된 비쥬얼 스튜디오의 버전들을 찾습니다.\n그리고 각 버전에 대해 WTL 마법사를 추가할 지 물어봅니다.");
	...
	{
		if(bDebug)
			WScript.Echo("찾기: " + astrVersions[i]);
	...
	if(bFound)
		MessageBox(WSShell, "완료!");
	else
		MessageBox(WSShell, "Setup은 설치된 비쥬얼 스튜디오를 찾지 못했습니다.");
}

function MessageBox(WSShell, strText, bYesNo)
{
	var nType = bYesNo ? (4 + 32) : 0;   // 4 = Yes/No buttons, 32 = Questionmark icon, 0 = OK button
	var nRetBtn = WSShell.Popup(strText, 0, "WTL 앱 마법사 Setup", nType);
...
function SetupWizard(WSShell, FileSys, strSourceFolder, strDestFolder, strWizVer, bWizSpecial, bExpress, bDebug)
{
	...
	catch(e)
	{
		...
		WScript.Echo("오류: 파일을 복사할 수 없습니다. (" + strError + ")");
		return;
	}
	...
	try
	{
		...
		if(fileSrc == null)
		{
			WScript.Echo("오류: 원본 파일을 열 수 없습니다. " + strSrc);
			return;
		}
		...
		if(fileDest == null)
		{
			WScript.Echo("오류: 대상 파일을 열 수 없습니다. " + strDest);
			return;
		}
	...
	catch(e)
	{
		...
		if(e.description.length != 0)
			strError = e.description;
		WScript.Echo("오류: WTLAppWiz.vsz 파일을 읽고 쓸 수 없습니다. (" + strError + ")");
		return;
	}
	...
	try
	{
		...
		if(bDebug)
			WScript.Echo("WTL 폴더: " + strDestWTLFolder);
	}
	catch(e)
	{
		var strError = "정보 없음";
		if(e.description.length != 0)
			strError = e.description;
		WScript.Echo("오류: WTL 폴더를 만들 수 없습니다. (" + strError + ")");
		return;
	}
	...
	catch(e)
	{
		...
		if(e.description.length != 0)
			strError = e.description;
		WScript.Echo("오류: WTL\\WTLAppWiz.vsdir 파일을 읽고 쓸 수 없습니다. (" + strError + ")");
		return;
	}

	WScript.Echo("WTL 마법사가 성공적으로 설치되었습니다!");
}
