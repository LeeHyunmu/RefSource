...
#include "stdafx.h"
#include "CouponPopDlg.h"

void CCouponPopDlg::init(UINT id)
{
	//
	bClose=FALSE;
	imgid=id;
}
void CCouponPopDlg::DlgClose()
{
	if(bClose==FALSE)
		PostMessage(WM_CLOSE);
	bClose=TRUE;
}
LRESULT CCouponPopDlg::OnInitDialog(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& /*bHandled*/)
{
	...
	if (img2.m_hWnd)
	{
		...
		if (m_wndBanner.Load(MAKEINTRESOURCE(IDR_GIF01),_T("GIF")))
			m_wndBanner.Draw();
	};
...
//|
//---------------------------------------------------------------
void CCouponPopDlg::OnMMTimer(UINT uID)
{
	if(bClose==FALSE)
		PostMessage(WM_CLOSE);
	bClose=TRUE;
}
LRESULT CCouponPopDlg::OnCloseCmd(WORD /*wNotifyCode*/, WORD wID, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
{
	m_Timer.StopTimer();
	EndDialog(wID);
	return 0;
...
//|
//---------------------------------------------------------------
BOOL CCouponPopDlg::PreTranslateMessage(MSG* pMsg)
{
...
//|
//---------------------------------------------------------------
void CCouponPopDlg::OnMouseMove(UINT nFlags, CPoint point)
{
...
//|
//---------------------------------------------------------------
void CCouponPopDlg::OnButtonClickedEvent(UINT uNotifyCode, int nID, CWindow wndCtl)
{
	switch(nID)
	{
...
//|
//---------------------------------------------------------------
LRESULT CCouponPopDlg::OnDestroy(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& /*bHandled*/)
{
	CMessageLoop* pLoop = _Module.GetMessageLoop();
	ATLASSERT(pLoop != NULL);
