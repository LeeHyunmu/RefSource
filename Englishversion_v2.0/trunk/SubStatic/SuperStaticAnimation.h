...
public:
	...
	//버튼 초기화 함수
	void initSuperStatic(UINT ImageId, UINT imgcount, BOOL bOwnerDraw=TRUE);
#ifdef WINCE
	void initSuperStatic(UINT ImageId, LPCWSTR type, UINT imgcount, BOOL bOwnerDraw=TRUE);
#endif //WINCE
	void initSuperStatic(CBitmapHandle bit, UINT imgcount, BOOL bOwnerDraw=TRUE);
	//Rgn  초기화 함수
	void initRgnSuperStatic(UINT ImageId, UINT MaskId, UINT imgcount, BOOL bOwnerDraw=TRUE);
	//Owner Draw 호출 함수
	...
	void StaticClear();
	//
	BOOL DivideHbitmap(CBitmapHandle hbit, UINT imgcount);
	//Rgn 
	BOOL RgnDivideHbitmap(CBitmapHandle hbit, UINT imgcount);
	//
	BOOL SetAnimation();
#ifdef WINCE
