...
public:
...
#endif //WINCE
	void initSuperButton(CBitmapHandle bit, BOOL bOwnerDraw=TRUE);
	//Rgn 버튼 초기화 함수
	...
	//
	void MakeButtonImage(CDC * pDC,const CBitmapHandle butimg);
	
	//
	void SetChangeXPos();
	//
	BOOL MoveXAnimation(int& xmove);
	//
	void SetChangeYPos(HDWP _hdwp);
	//
	BOOL MoveYAnimation(HDWP _hdwp, int& ymove);
#ifndef WINCE
	//버튼의 후버를 위한 함수
