...
// 	//이미지
// 	img_1.Create(hWnd, CRect(iX-50, 90, 0, 0),_T(""), WS_CHILD | WS_VISIBLE , 0, IDC_MODECHANGE);
// 	//이미지
// 	img_1.initSuperStatic(IDB_JPG1,_T("JPG"));

	// 병원 선택 버튼
	HospitalBut_1.Create(hWnd, CRect(399, 210, 0, 0),_T(""), WS_CHILD | WS_VISIBLE   , 0, IDC_HOSPITAL_1);
	//이미지(비트맵) 인자로 넘긴다
	HospitalBut_1.initSuperButton(IDB_PNG7, _T("PNG"));
	HospitalBut_1.ShowWindow(SW_HIDE);

	//병원 선택 버튼
	HospitalBut_2.Create(hWnd, CRect(399, 280, 0, 0),_T(""), WS_CHILD | WS_VISIBLE , 0, IDC_HOSPITAL_2);
	//이미지(비트맵) 인자로 넘긴다
	HospitalBut_2.initSuperButton(IDB_PNG8, _T("PNG"));
	HospitalBut_2.ShowWindow(SW_HIDE);

	//병원 선택 버튼
	HospitalBut_3.Create(hWnd, CRect(399, 350, 0, 0),_T(""), WS_CHILD | WS_VISIBLE , 0, IDC_HOSPITAL_3);
	//이미지(비트맵) 인자로 넘긴다
...
BOOL CMainGdiDlg::ImageChange(UINT _MgNum, BOOL bSendData)
{
	BOOL b=FALSE;
	//모드가 2일 때
	if(MgMode==2)
	...
	else
	{
		...
		{
			UINT FileType;
			FileType=MgMode==0 ? 0x00000002 : 0x00000001;
			//데이타를 보낸다 이미지 넘버 지금은 MgMode 값
			//TRUE를 리턴하면 이미지를 바꾸도록 한다
			if(dualip2p->SendDatafn(FileType, MobileSendData[MgMode]))
			{
				MgNum=_MgNum;
			}
		}
