...
//|
//---------------------------------------------------------------
void CTraceFileDebug::CreateTraceFile(TCHAR* name, BOOL is_truc)
{
	...
	while(OutputEvent.WaitForEvent())
	{
		...
		if(_filestream.is_open())
		{
			while (!qData.empty())
			{
				...
				_filestream << pdt.GetBuffer(pdt.GetLength()) << std::endl;
				qData.pop();
			}
		}
	}
