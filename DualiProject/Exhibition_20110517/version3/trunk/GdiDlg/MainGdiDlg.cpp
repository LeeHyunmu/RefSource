...
TCHAR MgFile0[][64]={_T("\\NandFlash\\coexsample\\image\\1.PNG"),
_T("\\NandFlash\\coexsample\\image\\2.PNG")
};
	...
	while(TRUE)
	{
		...
		else
		{
			...
			if(!bCardConnection&&dualicard->FindCard())
			{
				...
				if(b&&ImageNumber<MGFILECOUNT_0)
				{
					b = dualicard->ApduCommand(ApduCom[ImageNumber],scardRecv);
					imgbackup=ImageNumber; ImageNumber=ImageNumber+MGFILECOUNT_0;
					InvalidateRect(hWnd, NULL, FALSE);
					//InvalidateRect를 강제로 바로 실행 하기 위해존제..
					UpdateWindow(hWnd);
					bCardConnection=TRUE;
					GASound::SoundPlay(_T("NandFlash\\coexsample\\alarm.wav"));
				}
			}
