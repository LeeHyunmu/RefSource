...
//======================================================//
	...
	//레코드 셋 설정(특정 값 가져 오기)
	virtual BOOL RecordsetSetting(UINT pagsize, TCHAR* strQuery, TCHAR* format, ...);
	//레코드 셋 설정(테이블 전체)
	virtual BOOL RecordsetSetting(TCHAR* tablename);
	//레코드 셋 포인터를 넘긴다
	virtual IADORecordset* GetRecordsetPoint();
	//트랜젝션 사용 쿼리
	//시작
