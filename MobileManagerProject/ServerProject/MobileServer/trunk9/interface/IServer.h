...
interface IServer
{
	...
	//소켓 초기화
	virtual BOOL WinSockInitialize()=0;
	//서버 Listen
	virtual BOOL ServerListen(u_short _nport)=0;
	//소켓 닫기
	virtual void fnCloseSocket()=0;
	//받는 함수
	virtual DWORD fnWSARecv(SOCKET hSock, DWORD dwRecv=0, DWORD dwFlags = 0)=0;
	//보내는 함수
	...
	//비동기 서버 소켓 접속
	virtual SOCKET CompletionPortServerSocket(HANDLE hIocp)=0;
	//비동기 클라이언트용 소켓 만들기 함수
	virtual SOCKET CompletionPortClientSocket(HANDLE hIocp, SOCKET hSock, CString _ip, u_short _port)=0;
};
