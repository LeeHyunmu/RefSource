...
//|
//---------------------------------------------------------------
SOCKET CIocpServer::CompletionPortServerSocket(HANDLE hIocp)
{
	CString     strIP="";
	SOCKET hClientSocket;
	HANDLE  hResult;
	...
	if(hResult == NULL)
	{
		closesocket(hClientSocket); hClientSocket=0;
		return hClientSocket;
	}
	// 비동기 입출력 시작
...
//---------------------------------------------------------------
/////////////////////////////////////////////////////////////
DWORD CIocpServer::fnWSARecv(SOCKET hSock, DWORD dwRecv, DWORD dwFlags)
{
	int iRetVal;
	if(!m_hSocket)
		return FALSE;
