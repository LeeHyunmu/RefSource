...
void CDatabasebMg::Trans(TCHAR* strQuery, TCHAR* format, ...)
{
	...
	BOOL b = adodb->Execute(Query);//, &vRecsAffected, adOptionUnspecified);
	if(b)
	{
		HRESULT hr;
...
//--------------------------------------------------------------
//|
//|VM 요청 데이타를 DB로부터 가져온다
//|
//---------------------------------------------------------------
IADORecordset* CDatabasebMg::GetDbRecordset(UINT pagsize, TCHAR* strQuery, TCHAR* format, ...)
{
	if(adodb->IsOpen()==FALSE)
	{
	...
	TCHAR Query[256]={0,};
	sprintf_s(Query, "%s %s",strQuery, buffer);
	IADORecordset *adorecord= obj->CreateAdoRecordset();
