...
#define  DBSERVER_PORT 9001
//=====================================================
// MS SQL ADO 연동 스트링(Database=Mobile에 접속)
//======================================================
#define MYBIDBCONNECT_STR  _T("Driver={SQL Server};Server=localhost;Trusted_Connection=Yes;Database=MybiDb;Initial Catalog=ADO;Max Pool Size=75;Min Pool Size=5;uid=sa;pdw=gusan5763")
...
void CDbServerMg::IocpServerRecvHandling(SOCKETINFO* _info, DWORD flag)
{
	...
	else
	{
		...
		//크기를 구한다
		bsdt->dwdatalen=sizeof(baseDB20)+sizeof(DBError36)-2; // 보낼 구조체 크기
		//======================================================//
	...
	if(iocp&&iocp->fnGetSockInfo(&socketinfo, sock))
	{
		...
		if(thdpool)
		{
			...
			if(dbsendobj)
			{
				...
				//	전송 코드 삽입	
				//=======================================================//
				bsdtclt->dwBaseCode|=MYBIDATABASECOMM_DATARECV;
				//스드래플 사용 오브젝트 초기화
