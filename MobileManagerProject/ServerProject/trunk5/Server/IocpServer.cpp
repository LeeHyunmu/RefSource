...
void CIocpServer::Release()
{
	...
	{
		::shutdown(SockInfo[i]->socklist.hSock, SD_BOTH);
		closesocket(SockInfo[i]->socklist.hSock);
...
DWORD CIocpServer::fnWSASendAll(SOCKET hSock, const void* lpBuf, DWORD dwSend, DWORD dwFlags)
{
	...
	for(int i=0; i < (int)SockInfo.size(); i++)
	{
		SOCKET hClientSock=SockInfo[i]->socklist.hSock;
		if(hSock!=hClientSock)
		{
			ZeroMemory(&SockInfo[i]->wsaOv, sizeof(SockInfo[i]->wsaOv));
			memcpy_s(SockInfo[i]->data,MAX_SOCKBUF,lpBuf, dwSend);
			SockInfo[i]->wsaBuf[1].buf = (char *)SockInfo[i]->data;
			SockInfo[i]->wsaBuf[1].len = dwSend;
			SockInfo[i]->wType[1] = IO_TYPE_SEND;
			iRetVal = WSASend(hClientSock, &SockInfo[i]->wsaBuf[1], 1, &dwSend, 0, (LPOVERLAPPED)&SockInfo[i], NULL);
			if(iRetVal == 0 )
			{
				// 정상적으로 한번에 보내기 성공
				debug->Write("정상적으로 한번에 보내기 성공");
...
#define  MB_PORT 9001
SOCKET CIocpServer::CompletionPortClientSocket(HANDLE hIocp, SOCKET hSock)
{
	CStaticDataInitCriticalSectionLock  m_cs;
	m_cs.Lock();
	...
	cIntArrd.sin_addr.s_addr = inet_addr(MB_IP);
	cIntArrd.sin_port = htons(MB_PORT);
	//connect
