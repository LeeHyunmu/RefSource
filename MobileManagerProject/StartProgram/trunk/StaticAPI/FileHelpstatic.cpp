...
//#define  _MODULENAME_
#define LOGROOT _T("C:\\WINDOWS\\Anyq")
#define LOGPATH _T("C:\\WINDOWS\\Anyq\\log")
int CFileHelpstatic::logfile(TCHAR* Format,...)
{
	CStaticDataInitCriticalSectionLock sec;
	sec.Lock();
#ifdef _LOGFILE
	va_list		ap;
	TCHAR		sBuf[MAX_PATH];
	memset(sBuf, 0x00, sizeof(TCHAR)*MAX_PATH);
	va_start(ap, Format); 
	_vstprintf_s(sBuf, sizeof(TCHAR)*MAX_PATH, Format, ap);
	va_end(ap);
	TCHAR CurrentName[MAX_PATH];
	memset(CurrentName, 0x00, sizeof(TCHAR)*MAX_PATH);
	GetModuleFileName(0,CurrentName,sizeof(CurrentName));
	TCHAR* ActiveName =_tcsrchr(CurrentName,'\\');
	ActiveName++;
	...
	t=time(NULL);
	tm = localtime(&t);
	TCHAR	wrt_buff[HKM_BUF+1];
	TCHAR	wrt_file[100];
	memset(wrt_file, 0x00, sizeof(wrt_file));
#ifndef _MODULENAME_
	_stprintf(wrt_file, _T("%s\\%s_LOG%04d%02d%02d.log"),LOGPATH,ActiveName, 1900+tm->tm_year, tm->tm_mon + 1, tm->tm_mday);
#else
	_stprintf(wrt_file, _T("%s_LOG%04d%02d%02d.log"),CurrentName, 1900+tm->tm_year, tm->tm_mon + 1, tm->tm_mday);
#endif
	if(!FindFileAndFolder(wrt_file))
	{
		if(!FindFileAndFolder(LOGPATH))
		{
			CreateDirectory(LOGROOT,NULL);
			CreateDirectory(LOGPATH,NULL);
		}
		else
			DirectoryFindFileDelete(LOGPATH,ActiveName);
	}
	memset(wrt_buff, 0, sizeof(wrt_buff));
	wsprintf(wrt_buff,_TEXT("%04d-%02d-%02d %02d:%02d:%02d %s"), 1900+tm->tm_year, tm->tm_mon + 1, tm->tm_mday, 
		tm->tm_hour, tm->tm_min, tm->tm_sec,sBuf);
	CreateWriteFile(wrt_file,wrt_buff);
#else
	_tcprintf(_T("%s_%s\n"),ActiveName, sBuf);
#endif
#endif
