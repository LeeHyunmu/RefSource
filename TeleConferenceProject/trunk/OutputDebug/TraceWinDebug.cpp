...
void CTraceWinDebug::Release()
{
	...
	if(OutputEvent.IsOpen())
	{
		...
		CloseWinDebugEvent.WaitForEvent();
	}
	...
	while(OutputEvent.WaitForEvent())
	{
		...
		if(_window&&_listbox)
		{
			...
			while (!qData.empty())
			{
				...
				{
					MessageBox(NULL, _T("1"),_T("ok"),MB_OK);break;
				}
			}
			// 리스트 박스의 컨텐츠 개수를 구한다.
			if(SUCCEEDED(SendMessageTimeout( _listbox, LB_GETCOUNT, 0, 0L ,SMTO_ABORTIFHUNG,500,&dwresult)))
			{
				...
				if(SUCCEEDED(SendMessageTimeout( _listbox, LB_SETCURSEL, (WPARAM)n, 0L,SMTO_ABORTIFHUNG,500,&dwresult )))
				{
