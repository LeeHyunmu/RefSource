...
//|
//---------------------------------------------------------------
BOOL GAFileStream::CreateByteWriteFile(CString filename, char* bytefile, DWORD dwFileSize)
{
	HANDLE hFile;
	hFile = CreateFile(filename.GetBuffer(filename.GetLength()),GENERIC_WRITE,FILE_SHARE_WRITE,NULL,OPEN_ALWAYS,
		FILE_ATTRIBUTE_NORMAL, NULL);
	if(hFile!=INVALID_HANDLE_VALUE)
	{
		DWORD dwWritten;
		WriteFile(hFile, bytefile, dwFileSize ,&dwWritten,NULL);
		CloseHandle(hFile);
		return TRUE;
	}
	return FALSE;
