	...
	while(1)
	{
		...
		for(int i=0; i<nTotalSockets; i++)
		{
			...
			if(FD_ISSET(ptr->sock, &rset))
			{
				// 데이터 받기
				if(ptr->maxrecv==0)
					ptr->maxrecv=FIRSTTCPSERVER_RECVSIZE;
	...
	while(1)
	{
		...
		if(FD_ISSET(hUdpSocket, &rset))
		{
			...
			if(udpinfo.maxrecv==0)
				udpinfo.maxrecv=FIRSTUDPSERVER_RECVSIZE;
	...
	while(1)
	{
		...
		if(FD_ISSET(hTcpClientSocket, &rset))
		{
			...
			if(tcpclientinfo.maxrecv==0)
				tcpclientinfo.maxrecv=FIRSTTCPCLIENT_RECVSIZE;
	...
	while(1)
	{
		...
		if(FD_ISSET(hUdpClientSocket, &rset))
		{
			...
			if(udpclientinfo.maxrecv==0)
				udpclientinfo.maxrecv=FIRSTUDPCLIENT_RECVSIZE;
