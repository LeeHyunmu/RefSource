...
//	m_fRoll+= 1; // Add some rotation to the cube
	...
	if(glObject)
	{
		glObject->idleRotatecall(KeyValue, m_fPithc, m_fRoll, m_fYaw);
		glObject->idleTranslatecall(KeyValue, transX, transY, transZ);
	}
	RedrawWindow();
	return FALSE;
}
...
void CSuperOpenGLStatic::OnInit(void) 
{
	...
	glClearColor(0.000f, 0.500f, 0.200f, 1.0f); //Background color
...
void CSuperOpenGLStatic::OnResize(int cx, int cy)
{
	...
	GLfloat fZFar = 50.0f;  // Far clipping plane
...
//|
//---------------------------------------------------------------
void CSuperOpenGLStatic::SetVirtualKey(UINT _KeyValue)
{
	KeyValue=_KeyValue;
}
//--------------------------------------------------------------
