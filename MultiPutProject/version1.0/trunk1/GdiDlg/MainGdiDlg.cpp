	...
	else
	{
		...
		else if(SERVERFLAGE_SENSOREVENTHEAD==((BYTE)buf[0]))
		{
			...
			else if(recvbytes==14)
			{
				BYTE SensorType;
				int datepoint=1;
				//memcpy(&RecvType, &buf[datepoint], sizeof(BYTE));
				...
				if(SensorType==SENSORFLAGE_ORIENTATION)
				{
					...
					Yaw=(float*)&fdata[0];
					//오픈지일 좌표를 입력
					opengl.SetPitchRollYaw(*Pitch*-1.0f, *Roll*-1.0f, *Yaw*-1.0f);
				}
				//다음 데이터를 받기 위한 초기화
				recvbytes = maxrecv = sendbytes = totalsend = 0;
				TcpServerBufFlag = SERVERFLAGE_IDLESERVER;
...
void CMainGdiDlg::initDlg(HWND _hWnd)
{
	...
	opengl.Create(hWnd, CRect(20, 80, 600, 800),_T("UDP서버"), WS_CHILD | WS_VISIBLE , 0, IDC_UDPSERVER);;
