	...
	while(OutputEvent.WaitForEvent())
	{
		...
		if(_window&&_listbox)
		{
			while (!qData.empty())
			{
				...
				// 리스트 박스에 문자열을 추가한다.
				//SendMessageTimeout(_listbox, LB_ADDSTRING, 0, (LPARAM) pdt, SMTO_NORMAL, 1000, &dwvalue);
				// 리스트 박스의 컨텐츠 개수를 구한다.
				//SendMessageTimeout( _listbox, LB_GETCOUNT, 0, 0L, SMTO_NORMAL, 1000, &dwvalue );
				DWORD n = dwvalue-1;
				// 리스트 박스의 컨텐츠 개수만큼 커서를 아래로 이동시킨다.
				//SendMessageTimeout( _listbox, LB_SETCURSEL, (WPARAM)n, 0L, SMTO_NORMAL, 1000, &dwvalue );
				qData.pop();
			}
		}
		else
