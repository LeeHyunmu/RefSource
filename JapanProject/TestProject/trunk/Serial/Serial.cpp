...
{
	hCom = INVALID_HANDLE_VALUE;  // 기본값
	   hCom=CreateFile(szPort ,                           // 포트명을 지정
		GENERIC_READ|GENERIC_WRITE,    // 장치 접근 모드 지정
		0,                                 // 현장치 사용 권한
		NULL,                              // NULL
		OPEN_EXISTING,                     // 장치 생성 모드
		FILE_ATTRIBUTE_NORMAL,             // 장치 속성 지정
		NULL);                             // NULL
...
//데이터를 보냄
BOOL CSerial::PortWrite(const byte *szData, int szDatalen)
{
	//Check Port
	if(hCom == INVALID_HANDLE_VALUE) return FALSE;
	...
	BOOL bRes = WriteFile(hCom ,      // 장치 핸들
		szData,     // 송신데이터
		szDatalen,  // 송신데이터 바이트수 
		&dwBytes,    // 보낸 바이트 수
		NULL);          // NULL
		//전송된 데이터 성공 여부 확인
		if(!(bRes == TRUE &&  szDatalen == dwBytes) ) return FALSE;
