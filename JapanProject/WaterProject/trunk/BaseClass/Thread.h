...
public:
   ...
   HANDLE m_hCloseEvent;	// 종료 이벤트
   DWORD m_dwThreadID;		// Thread ID
   bool m_bSuspended;		// Thread currently suspended?
   bool m_bLoop;			// Thread Loop 플래그
   

   CThreadT(HANDLE hThread = NULL) : 
   m_hThread(hThread),
	   m_hCloseEvent(INVALID_HANDLE_VALUE),
	   m_dwThreadID(0),
	   m_bSuspended(false),
	   m_bLoop(TRUE)
   {
   ...
   BOOL Release()
   {
	   ...
	   m_bLoop=FALSE;
	   if(m_hCloseEvent!= INVALID_HANDLE_VALUE)
	   {
		   if(!(::WaitForSingleObject(m_hCloseEvent, 500) == WAIT_OBJECT_0))
		   {
			   Terminate();
		   }
   ...
   {
      if( m_hThread == NULL || m_bLoop==FALSE ) return FALSE;
      DWORD dwCode = 0;
   ...
   BOOL SetCloseEvent()
   {
		ATLASSERT(m_hCloseEvent!=INVALID_HANDLE_VALUE);
		return ::SetEvent(m_hCloseEvent);
   }
