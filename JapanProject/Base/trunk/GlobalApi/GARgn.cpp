		...
		for(y = 0; y < bmpInfo32.bmHeight; y++) 
		{
			for(x = 0; x < bmpInfo32.bmWidth; x++)
			{
				...
				if(x > nStartX) 
				{
					...
					{
						CRgn NewRgn;
						if(NewRgn=::ExtCreateRegion(NULL, sizeof(RGNDATAHEADER) + (sizeof(RECT) * dwMaxRect), pRgnData))
						{
...
//|
//---------------------------------------------------------------
BOOL GARgn::ImageLoadRgn(HMODULE hModule,UINT ResourceName, LPCTSTR ResourceType, CRgnHandle& rgn,  HWND hWnd)
{
	BOOL bResult = FALSE;
	...
	{        
			...
			(void **)&pImgFactory)))
		{
			...
			if(h32BitBmp.CreateDIBSection(hMemDC,(BITMAPINFO*)&Header32Bit,DIB_RGB_COLORS,&pBmpBits,NULL,0))
			{
				...
				::SelectObject(hMemDC, hOldBmp);
				if(SetShapeBitmap(h32BitBmp,RGB(254,254,254),RGB(255,255,255),rgn))
					bResult=TRUE;
				h32BitBmp.DeleteObject();
			}
			::DeleteDC(hMemDC);
			::DeleteDC(hdc);
			pImage->Release();
