...
BOOL CAQdeviceCmd::initAQdeviceCmd(HWND _hWnd, LPCTSTR dlllib)
{
	...
	if(dlllib && *dlllib)
	{
		...
		if(m_module) 
		{
			...
			FindCardEvent.Create();
			EventProcEvent.Create();
			bLoadModule = TRUE;
		}
	}
	return bLoadModule;
...
void CAQdeviceCmd::NFCDisConnect()
{
	if( bLoadModule && m_module!=NULL)
	{
		...
		FindCardEvent.WaitForEvent(200);
		if(EventProcThread.IsRunning())
		{
			EventProcThread.Release();
		}
		EventProcEvent.WaitForEvent(200);
		bLoadModule=FALSE;
	}
	if(bNfcConnectOk)
	{
