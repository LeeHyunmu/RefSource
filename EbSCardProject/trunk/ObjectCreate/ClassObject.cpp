
#include "stdAfx.h"
#include "../OutputDebug/OutputDebug.h"
#include "../GdiDlg/GdiDlg.h"
#include "../SCardDevice/SCardDevice.h"
#include "../Client/Client.h"

//--------------------------------------------------------------
//|
//|생성자
//|
//---------------------------------------------------------------
CClassObject::CClassObject():
debug(NULL),
maingdi(NULL),
scardtm(NULL),
client(NULL),
mKillthread(NULL),
mhKillEvent(NULL)
{}
//--------------------------------------------------------------
//|
//|소멸자
//|
//---------------------------------------------------------------
CClassObject::~CClassObject()
{
}
//--------------------------------------------------------------
//|
//|사용중인 객체를 메모리에서 제거한다
//|
//---------------------------------------------------------------
void CClassObject::KillObject()
{
	////////////////////////////////////////////////////////////////
	//종료 스레드 생성
	mhKillEvent.Create();
	mKillthread.Create(KillProcess, (LPVOID)this);
	if(mhKillEvent.WaitForEvent())
	{
	}
	mKillthread.WaitForThread();
	//디버그 출력 창을 제일 나중에 종료한다
	if(debug)
	{
#ifdef _DEBUG
		delete ((CTraceWinDebug*)debug);	debug=NULL;
#else
		delete ((CTraceFileDebug*)debug);	debug=NULL;
#endif
	}
}
//--------------------------------------------------------------
//|
//|사용중인 객체를 메모리에서 제거한다
//|
//---------------------------------------------------------------

//--------------------------------------------------------------
//|
//| 메모리를 해제하기 위한 스래드
//|
//---------------------------------------------------------------
DWORD WINAPI CClassObject::KillProcess(LPVOID lpArg)
{
	IMyObject *obj = (IMyObject *)lpArg;
	if(obj)
		obj->Release();
	return 0;
}
//--------------------------------------------------------------
//|
//|사용중인 객체를 메모리에서 제거한다
//|
//---------------------------------------------------------------
void CClassObject::Release()
{
	//|=============================================
	//|MainDlg Gdi 클래스
	//|==================================================
	if(maingdi)
	{
		maingdi->Release();
		delete ((CMainGdiDlg*)maingdi);	maingdi=NULL;
	}
	//|=============================================
	//|SCardTm 클래스 객체
	//|==================================================
	if(scardtm)
	{
		scardtm->Release();
		delete ((CSCardTm*)scardtm);	scardtm=NULL;
	}
	//|=============================================
	//|client 클래스 객체
	//|==================================================
	if(client)
	{
		client->ReleaseClient();
		delete ((CMobileClient*)client);	client=NULL;
	}
	//|=============================================
	//|사용중인 객체의 메모리를 모두 해제 했으면
	//|==================================================
	mhKillEvent.SetEvent();
}
//--------------------------------------------------------------
//|
//|선언된 객체를 생성한다
//|
//---------------------------------------------------------------
BOOL CClassObject::CreateObject()
{
#ifdef _DEBUG
	debug = new CTraceWinDebug(_T("Eb_Debug"));
#else
	debug = new CTraceFileDebug(_T("Eb_Debug"));
#endif
	//======================================
	// Main Dlg Gdi 클래스
	//======================================
	maingdi = new CMainGdiDlg;
	//======================================
	// SCardTm 클래스
	//======================================
	scardtm = new CSCardTm;
	//======================================
	// client 클래스
	//======================================
	client = new CMobileClient;
	return TRUE;
}
//--------------------------------------------------------------
//|
//|객체 초기화
//|
//---------------------------------------------------------------
BOOL CClassObject::initObject()
{
	//======================================
	// 클래스 초기화
	//======================================
	//======================================
	// Main Dlg Gdi 클래스 초기화
	//======================================
	if(maingdi)
	{
		maingdi->initInterfaceObject(this);
	}
	//======================================
	// SCardTm 클래스 초기화
	//======================================
	if(scardtm)
	{
		scardtm->initInterfaceObject(this);
	}
	//======================================
	// client 클래스 초기화
	//======================================
	if(client)
	{
		client->initInterfaceObject(this);
	}
	return TRUE;
}
//--------------------------------------------------------------
//|
//|생성된 클래스 인터페이스 객체 포인터 넘기는 함수
//|
//---------------------------------------------------------------
//--------------------------------------------------------------
//|
//|디버그 출력 클래스
//|
//---------------------------------------------------------------
IOutput* CClassObject::GetOutputObject()
{
	return debug;
}
//--------------------------------------------------------------
//|
//| MainDlg 클래스
//|
//---------------------------------------------------------------
IMainGdiDlg* CClassObject::GetMainGdiObject()
{
	return maingdi;
}
//--------------------------------------------------------------
//|
//| MainDlg 클래스
//|
//---------------------------------------------------------------
ISCardTm* CClassObject::GetSCardTmObject()
{
	return scardtm;
}
//--------------------------------------------------------------
//|
//|통신 클라언트 객체
//|
//---------------------------------------------------------------
IClient* CClassObject::GetClientObject()
{
	return client;
}