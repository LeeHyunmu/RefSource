	...
	else
	{
		...
		if (m_pGIFLSDescriptor->GetPackedValue(LSD_PACKED_GLOBALCT) == 1)
		{
			...
			m_nGlobalCTSize = static_cast<int>
				(3*pow((float)2,(float)m_pGIFLSDescriptor->GetPackedValue(LSD_PACKED_GLOBALCTSIZE)+1));
...
BOOL CPictureExWnd::Load(LPCTSTR szFileName)
{
	...
	HGLOBAL hGlobal;
	DWORD dwSize;
	errno_t eno = _tfopen_s(&file, szFileName, _T("rb"));
	if( file == NULL || eno != 0 )         
	{             
...
int CPictureExWnd::GetNextBlockLen() const
{
	...
	case BLOCK_IMAGE:
		...
		int nLCTSize = (int)
			...
			pow((float)2,(float)pIDescr->GetPackedValue(ID_PACKED_LOCALCTSIZE)+1));
